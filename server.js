    ...
    inquirer.prompt({
        ...
        switch(ans.choice){
            ...
            case 'view all employees':
                db.query('SELECT employee.id, employee.first_name, employee.last_name,title,salary,manager.first_name AS manager               from employee LEFT JOIN role on role.id=employee.role_id LEFT JOIN employee manager ON manager.id=employee.manager_id;', (err, results) => {
                    if(err){
                        throw err;
                    } else {
                        if(!results.length){
